...
Stack:     encounter (awaiting user triage, encountered 3 times)
Tags:      [business, ChibaTech, approval-process]
```

- `confidence` is the LLM's self-reported assessment (0-1) of whether the utterance reflects genuine confusion versus conversational hedging or politeness. It is stored for analytics but does not gate flashcard creation -- all detected terms enter the encounter stack.
...
- **Encounter stack** -- all terms detected from clarification moments and passive summary capture land here. The stack persists indefinitely and accumulates across meetings. There is no pressure to attend to it -- it is a reservoir of encountered vocabulary, not a to-do list. Encounters are ordered by **frequency** (most frequently encountered terms at the top), so the participant triages the most common unknowns first. Terms already in the review stack do not accumulate here -- only new, unreviewed terms appear.
...
The participant **triages** the encounter stack with four swipe directions:

| Swipe | Meaning | Effect |
|-------|---------|--------|
| **Down** | "I don't know this -- study it" | Moves to review stack, initialised as `Rating.Again` |
| **Left** | "I don't know this -- but not now" | Moves to unknown discard pile (can be recovered later) |
| **Right** | "I already know this" | Moves to review stack with `Rating.Easy` (low starting repetition frequency) and marked as known |
| **Up** | "This is a duplicate" | System finds the original encounter and merges metadata (additional context sentences, audio snippets, meeting dates) into the original card for richer review |

This triage interaction is lightweight and can happen in small batches -- after a meeting, during a commute, or whenever the participant has a moment.

The separation is deliberate: the participant controls the balance between **dialing in existing knowledge** (review stack -- repeated FSRS practice on accepted terms) and **discovering new vocabulary** (encounter stack -- browsing and triaging new captures). Neither stack pressures the other. A participant who is overwhelmed with review can ignore encounters entirely; one who is hungry for new material can triage aggressively.
